					...
					<div class="side-app">
						...
						<?php 
						 ...
						 $approved="N";
							...
							$pid=@$_GET['pid'];
							$uid=@$_GET['uid'];
							$rdx=mysql_fetch_assoc(mysql_query("select * from tblupfront where id=$uid"));
							$uwef=$rdx['wef'];
							$uwet=$rdx['wet'];
							...
							if($logid==""){
								//echo "exists";exit;
								$res=mysql_query("insert into tblrunlogup(payroll,tyear,tmonth,tday,nextrun,transdate,runby) value($pid,$yr,$mnt,$tday,'$nextrun','$dd',$curuserid)");
								$logid=mysql_insert_id();
							}
							
							$assignedto=returnQueryValue("select assignedto from tblrunlogup where id=$logid","assignedto");
							...
							if($approved=="N"){
								...
								$pck.="<div class=\" text-dark\"><p class=\"mb-1 mt-7\"><span class=\"font-weight-semibold\" style='font-size:30px;'><u>Upfront Payment</u></span></p>";
								...
								$totEarning=$totEarning+$basicamnt;
									$pck.="<tr>";
										...
										$pck.="</tr>";
										$commit=savePayElementLogup("BASIC",$logid,"C",$tday,$mnt,$yr,$empidid,$basicamnt,"G","BASIC Pay","N","Basic Salary");
									
									$eleqry="select * from tblpayelement,tblupfrontitems where tblpayelement.active='Y' and tblpayelement.grade=$empgrade and tblupfrontitems.gradeid=tblpayelement.grade and
									creditdebit='C' and crita not in ('TAXG','PENS') and payroll=$pid and tblupfrontitems.elementid=tblpayelement.id";
									//echo $eleqry;
									...
									//	$amt=proratepay($empidid,$amt,$mnt,$yr);
										$amt=prorateupfront($uid,$empgrade,$amt,$eleid);
										...
										$pck.="</tr>";
										$commit=savePayElementLogup($eleid,$logid,"C",$tday,$mnt,$yr,$empidid,$amt,"G",$pelemnarr,"N",$payelement);
							...
							////////////////////Personaized angle for credit		
									$pqry="select * from tblemployeepayelement where 1=2";
								//	echo $pqry;
									...
									if($pnum>0){
										do{
											...
											$amount=prorateupfront($uid,$empgrade,$amount,$empidid);
											$totEarning=$totEarning+$amount;
											$pck.="	<td>$payelement<br></td><td>".number_format($amount,2)."</td>";
										$pck.="</tr>";
										$commit=savePayElementLogup("CUSTOM".$pesid,$logid,"C",$tday,$mnt,$yr,$empidid,$amount,"P","Use Amount","N",$payelement);
										//echo $commit."<br>";
						...
						////////////////////////////////////////////////////////////////
								...
								$totdeduct=0;
									...
									$eleqry="select * from tblpayelement,tblupfrontitems where tblpayelement.grade=$empgrade and creditdebit='D' and crita not in ('TAXG','PENS') and payroll=$pid and tblupfrontitems.elementid=tblpayelement.id and tblupfrontitems.gradeid=tblpayelement.grade";
									...
									if($retnop>0){
										...
										$commit=savePayElementLogup($eleid,$logid,"D",$tday,$mnt,$yr,$empidid,$amt,"G",$pelemnarr,"N",$payelement);
										//echo $commit."<br>";
							...
							////////////////////Personaized angle for DEBIT		
									$pqry="select * from tblemployeepayelement where 1=9";
									$pres=mysql_query($pqry);
									$prd=mysql_fetch_assoc($pres);
									$pnum=mysql_num_rows($pres);
									if($pnum>0){
										do{
											...
											$pck.="	<td>$payelement<br></td><td>".number_format($amount,2)."</td>";
										$pck.="</tr>";
										$commit=savePayElementLogup("CUSTOM".$pesid,$logid,"D",$tday,$mnt,$yr,$empidid,$amount,"P","Use Amount","N",$payelement);
						...
						////////////////////////////////////////////////////////////////
									...
									$pck.="<tr>";
										...
										$txqry="select * from tblpayelement,tblupfrontitems where tblpayelement.active='Y' and tblpayelement.grade=$empgrade and creditdebit='D' and crita in ('TAXG','PENS') and payroll=$pid and tblupfrontitems.elementid=tblpayelement.id and tblupfrontitems.gradeid=tblpayelement.grade";
										...
										if($txnum>0){
											do{
												...
												$txamt=prorateupfront($uid,$empgrade,$txamt,$peleid);
												$totgded=$totgded+$txamt;
												
												$crita=$txrd['crita'];
												$commit=savePayElementLogup($peleid,$logid,"D",$tday,$mnt,$yr,$empidid,$txamt,"G",$pelemnarr,"Y",$txname);
							...
							<div class="col-md-12">
								<div class="card">
									<div class="card-header">
										...
										?><a href="requestapprovalup.php?logid=<?php echo $logid; ?>"><button type="button" class="btn btn-sm btn-blue mr-2"><i class="icon ion-checkmark"></i> Request Payroll Approval</button></a>	
										<?php 
										}
										...
										<?php } } ?>
											...
											<?php echo "<a href='pdfpayslipup.php?logid=$logid' >"; ?><button type="button" class="btn btn-sm btn-secondary mr-2"><i class="fa fa-file-pdf-o"></i> Export to PDF</button><?php echo "</a>"; ?>
											<?php echo "<a href='excelslipup.php?logid=$logid' >"; ?><button type="button" class="btn btn-sm btn-secondary mr-2"><i class="fa fa-file-excel-o"></i> Export to Excel</button></a>
											<?php } ?>
											
											<a href="exportallupfront.php?logid=<?php echo $logid; ?>" target="_blank"><button type="button" class="btn btn-sm btn-blue mr-2"><i class="icon fa-file-excel-o"></i> Export All to CSV</button></a>
										</div>
									</div>
									<?php 
										echo $pck;
