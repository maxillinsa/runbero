<!doctype html>
<html lang="en" dir="ltr">
	<head>
		<?php include("header.php"); ?>

				<div class="app-content  my-3 my-md-5">
					<div class="side-app">
						<div class="page-header">
							<h4 class="page-title">Loan Management</h4>
							<ol class="breadcrumb">
								<li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
								<li class="breadcrumb-item active" aria-current="page">Loans</li>
							</ol>
						</div>

						
							<div class="col-md-12 col-lg-12">
								<div class="card">
									<div class="card-header">
										<div class="card-title">Loans Statuses</div>

									</div>
									
									<div class="card-body">
									
									<?php 
										$qry="select * from  tblloans where compid=$compid and approved='Y'";
									//	echo $qry;
										$res=mysql_query($qry);
										$nm=mysql_num_rows($res);
										if($nm<1){
											echo "No record found<br>";
										}else{
											$rd=mysql_fetch_assoc($res);
											$ip=1;
											
											
									
									
									?>
										<div class="table-responsive">
											<table id="example2" class="hover table-bordered border-top-0 border-bottom-0" >
												<thead>
													<tr>
														<th>S/N</th>
														<th>Loan Type</th>
														<th>Duration</th>
														<th>Rate</th>
														<th>Burrower</th>
														<th>Principal</th>
														<th>Interest</th>
														<th>Total Amount</th>
														<th>Amount Repaid</th>
														<th>Loan Balance</th>
														<th></th>
													</tr>
												</thead>
												<?php 
													do{
														$duration=$rd['duration'];
														$empid=$rd['empid'];
														$principal=$rd['principal'];
													$interest=$rd['interest'];
													$amount=$rd['amount'];
													$id=$rd['id'];
													$amtrepaid=returnQueryValue("select sum(instamount)instamount from tblloanshedule where id=$id and paid='Y'","instamount");
												if($amtrepaid==""){
													$amtrepaid=0;
												}
												
												$lnbalance=$amount-$amtrepaid;
												?>
												<tbody>
													
													<tr>
														<td><?php echo $ip; ?></td>
														<td><?php 
														
											$lntid=$rd['loanid'];
											$loantype=returnQueryValue("select name from loantypes where id=$lntid","name");
											
														echo $loantype; ?></td>
														<td><?php echo $rd['duration']; ?></td>
														<td><?php echo $rd['intrate']; ?></td>
														
														<td><?php 
														$cby=$rd['createdby'];
														$ename=returnQueryValue("select fullname from tblemployee where id=$empid","fullname");
														echo $ename; ?></td>
														<td><?php echo number_format($principal,2); ?></td>
														<td><?php echo number_format($interest,2); ?></td>
														<td><?php echo number_format($amount,2); ?></td>
														<td><?php echo number_format($amtrepaid,2); ?></td>
														<td><?php echo number_format($lnbalance,2); ?></td>
															<td>
														<a href='loandoc.php?id=<?php echo $id; ?>' target='_blank'><button class='btn btn-sm btn-primary'>Documents<i class='fa fa-file-text-o'></i></button> </a>
														<a href='loanschedule.php?id=<?php echo $id; ?>' target='_blank'><button class='btn btn-sm btn-info'>View Schedule</i></button> </a>
														</td>
														
													</tr>
												</tbody>
												<?php 
												
												$ip=$ip+1;
													}
													while($rd=mysql_fetch_assoc($res));
										}
												
												?>
											
											</table>

										</div>
									</div>
								
								
								</div>
							</div>
						
					
					</div>
				</div>
			</div>

			<!--footer-->
		<?php include("footer.php"); ?>
			<!-- End Footer-->
		</div>

		<!-- Back to top -->
		<a href="#top" id="back-to-top" ><i class="fa fa-rocket"></i></a>


		<!-- Dashboard Core -->
		<script src="../assets/js/vendors/jquery-3.2.1.min.js"></script>
		<script src="../assets/plugins/bootstrap/js/popper.min.js"></script>
		<script src="../assets/plugins/bootstrap/js/bootstrap.min.js"></script>
		<script src="../assets/js/vendors/jquery.sparkline.min.js"></script>
		<script src="../assets/js/vendors/selectize.min.js"></script>
		<script src="../assets/js/vendors/jquery.tablesorter.min.js"></script>
		<script src="../assets/js/vendors/circle-progress.min.js"></script>
		<script src="../assets/plugins/rating/jquery.rating-stars.js"></script>

		<!--Counters -->
		<script src="../assets/plugins/counters/counterup.min.js"></script>
		<script src="../assets/plugins/counters/waypoints.min.js"></script>

		<!-- Fullside-menu Js-->
		<script src="../assets/plugins/toggle-sidebar/sidemenu.js"></script>

		<!-- CHARTJS CHART -->
		<script src="../assets/plugins/chart/Chart.bundle.js"></script>
		<script src="../assets/plugins/chart/utils.js"></script>

		<!-- Custom scroll bar Js-->
		<script src="../assets/plugins/scroll-bar/jquery.mCustomScrollbar.concat.min.js"></script>

		<!-- ECharts Plugin -->
		<script src="../assets/plugins/echarts/echarts.js"></script>
		<script src="../assets/plugins/echarts/echarts.js"></script>
		<script src="../assets/js/index1.js"></script>
		
		<script src="../assets/plugins/datatable/jquery.dataTables.min.js"></script>
		<script src="../assets/plugins/datatable/dataTables.bootstrap4.min.js"></script>
		<script src="../assets/js/datatable.js"></script>

		<!-- Select2 js -->
		<script src="../assets/plugins/select2/select2.full.min.js"></script>

		<!-- Custom scroll bar Js-->
		<script src="../assets/plugins/scroll-bar/jquery.mCustomScrollbar.concat.min.js"></script>


		<!--Counters -->
		<script src="../assets/plugins/counters/counterup.min.js"></script>
		<script src="../assets/plugins/counters/waypoints.min.js"></script>

		<!-- Custom Js-->
		<script src="../assets/js/admin-custom.js"></script>

	</body>
</html>