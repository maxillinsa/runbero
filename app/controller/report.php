<?php 
	...
	//	}
		...
		//echo $qry;exit;
			...
			do{
				...
				$ret.="<td>".number_format($rd['openbalance'],4)."</td>";
				$ret.="<td>".number_format($rd['withdrawalamount'],2)."</td>";
				$ret.="<td>".number_format($rd['intopenbalance'],4)."</td>";
				$ret.="<td>".number_format($rd['contrib'],2)."</td>";
				$ret.="<td>".number_format($rd['intcontrib'],4)."</td>";
				$clsBal=$rd['openbalance']+$rd['intopenbalance']+$rd['contrib']+$rd['intcontrib'];
				$ret.="<td>".number_format($clsBal,4)."</td>";
	...
	if($act=="viewstatementofaccountself"){
		...
		$ret.="<table id='dynamic-table' class='table table-striped table-bordered table-hover'><thead>";
		$ret.="<tr>";
		$ret.="<th>Opening Balance</th><th>Withdrawal</th><th>Interest on Opening Balance</th><th>Contribution</th><th>";
		$ret.="<i class='ace-icon fa fa-clock-o bigger-110 hidden-480'></i>";
		$ret.="Interest on Contribution";
		$ret.="</th>";
		$ret.="<th class='hidden-480'>Closing Balance</th>";
...
if($num>0){
		...
		//	echo $staffid." min_month=".$min_month." max_month=".$max_month." min_year=".$min_year." max_year=".$max_year."<br>";
			for ($xy = $min_year; $xy <= $max_year; $xy++) {
				for ($xm = $min_month; $xm <= $max_month; $xm++) {
					...
					$nm=recNum("select * from tblallocation where staffid='$staffid' and mnt=$xm and tyear=$xy");
					if($nm>0){
						$i+=1;
			...
			//echo "select sum(withdrawalamount)withamnt from tblallocation where staffid='$staffid' and mnt=$xm and tyear=$xy and comment like '%ithdrawal%';";
				$withamntall=returnQueryValue("select sum(withdrawalamount)withamnt from tblallocation where staffid='$staffid' and mnt=$xm and tyear=$xy and comment like '%ithdrawal%'","withamnt");
			//	$ret.="<td class='center'><label class='pos-rel'><input type='checkbox' class='ace' /><span class='lbl'></span></label></td>";
				$ret.="<td>".number_format($openingBalance ,2)."</td>";
				$ret.="<td>".number_format($withamntall ,2)."</td>";
	...
	if($act=="viewstatementofaccountself2"){
		...
		$ret.="<table id='dynamic-table' class='table table-striped table-bordered table-hover'><thead>";
		$ret.="<tr>";
		$ret.="<th style='color:#022E63;'>Opening Balance</th><th style='color:#022E63;'>Withdrawal</th><th style='color:#022E63;'>Interest on Opening Balance</th><th style='color:#022E63;'>Contribution</th><th style='color:#022E63;'>";
		$ret.="<i class='ace-icon fa fa-clock-o bigger-110 hidden-480'></i>";
		$ret.="Interest on Contribution";
		$ret.="</th>";
		$ret.="<th class='hidden-480' style='color:#022E63;'>Closing Balance</th>";
		
		$ret.="<th style='color:#022E63;'>Date</th>";
...
if($num>0){
	...
	if($act=="viewstatementofaccountself2liqudation"){
		//$curuser=@$_COOKIE['curuser'];
		...
		//	echo $staffid." min_month=".$min_month." max_month=".$max_month." min_year=".$min_year." max_year=".$max_year."<br>";
			for ($xy = $min_year; $xy <= $max_year; $xy++) {
				for ($xm = $min_month; $xm <= $max_month; $xm++) {
					...
					if($rnm>0){
						//echo "select * from tblallocation where staffid='$staffid' and mnt=$xm and tyear=$xy ORDER BY id ASC LIMIT 1";exit;
					$rdo=mysql_fetch_assoc(mysql_query("select * from tblallocation where staffid='$staffid' and mnt=$xm and tyear=$xy ORDER BY id ASC LIMIT 1"));
					$openingBalance= $rdo['openbalance'];
				...
				//	echo $intcontribute;
					$fid=$rdo['id'];
					$withamntall=returnQueryValue("select sum(amount)withamnt from tblwithdrawal where staffid='$staffid' and mnt=$xm and tyear=$xy","withamnt");
					$withamnt1=returnQueryValue("select sum(amount)withamnt from tblwithdrawal where allocid=$fid","withamnt");
					if($withamnt1==""){
						$withamnt1="0";
					}
...
if($num>0){
	...
	do{
		...
		//	echo $staffid." min_month=".$min_month." max_month=".$max_month." min_year=".$min_year." max_year=".$max_year."<br>";
			...
			//	$ret.="<td>".$staffid."</td>";
				//$ret.="<td>".$staffname."</td>";
			//	$ret.="<td>".$remark."</td>";
				$startinterest=returnQueryValue("select intopenbalance from tblallocation where staffid='$staffid' and mnt=$xm and tyear=$xy order by id desc","intopenbalance");
				$openingBalance=$openingBalance-$startinterest;
				$withamntall=returnQueryValue("select sum(withdrawalamount)withamnt from tblallocation where staffid='$staffid' and mnt=$xm and tyear=$xy and comment like '%ithdrawal%'","withamnt");
				//$openingBalance=$openingBalance-$withamntall;
				//$openingBalance=$openingBalance-$withamntall;
				//$openingBalance=$openingBalance+$withamntall;
				...
				$withamnt1_2=$withamnt1_2+$withamntall;
				$withamntall_2=$withamntall_2+$withamntall;
				//$clsBal=round($rd['openbalance']+$rd['intopenbalance']+$rd['contrib']+$rd['intcontrib'],2);
				//$ret.="<td>".number_format($ClosingBalance,2)."</td>";
	...
	while($rd=mysql_fetch_assoc($res));
		//$txtstartmonth=$_POST['txtstartmonth'];
		//$txtyear=$_POST['txtyear'];
		//$openingBalance_2=$openingBalance_2+$withamnt1_2;
	$ddx="01/".$txtstartmonth."/".$txtyear;
			...
			$ret.="<td><b>Interest</b></td><td> ".number_format($intesiti,2)."</td>";				
				$ret.="</tr>";
				$totobalanci=$openingBalance_2+$contribamt_2+$intesiti-$withamnt1_2;
				
					$ret.="<tr>";
			$ret.="<td><b>Closing Balance</b></td><td>".number_format($totobalanci,2)."</td>";				
				$ret.="</tr>";
...
if($num>0){
	...
	do{
		...
		$staffname=returnQueryValue("SELECT CONCAT(surname,' ',firstname)namer FROM tblmembers where staffid='$staffid'","namer");
		$remark=returnQueryValue("select payrollsource from tblmembers where staffid='$staffid'","payrollsource");
		$min_month=$txtstartmonth;
		$max_month=$txtstartmonth;
		$min_year=$txtyear;
		$max_year=$txtyear;
		if(!empty($min_month)){
			$i=0;
		//	echo $staffid." min_month=".$min_month." max_month=".$max_month." min_year=".$min_year." max_year=".$max_year."<br>";
			for ($xy = $min_year; $xy <= $max_year; $xy++) {
				...
				//	echo $intcontribute;
					...
					$rdo2=mysql_fetch_assoc(mysql_query("select (openbalance+intopenbalance)closingbalance,openbalance,investdate,intopenbalance,contrib,id from tblallocation where staffid='$staffid' and mnt=$xm and tyear=$xy ORDER BY id desc LIMIT 1"));
					$ClosingBalance= $rdo2['closingbalance'];
					$contribamt2=$rdo2['contrib'];
					$ddx=$xy."-".$xm."-20";
					$lastday =date("t", strtotime($ddx));
					$nminvdays=(int)$lastday -21;
					$nopBalance=$rdo2['openbalance'];
	...
	if($act=="loaddepositreport"){
		$txtdate=$_POST['txtdate'];
		$txtdate2=$_POST['txtdate2'];
		$cbosource=@$_POST['cbosource'];
		$txtdate=date('Y-m-d', strtotime(str_replace('/', '-', $txtdate)));
		$txtdate2=date('Y-m-d', strtotime(str_replace('/', '-', $txtdate2)));
		if(empty($cbosource)){
		$qry="select * from tbldeposits where paymentdate between '$txtdate' and '$txtdate2' and deleted='N' order by id desc";
		}else{
			$qry="select * from tbldeposits where paymentdate between '$txtdate' and '$txtdate2' and deleted='N' and source='$cbosource' order by id desc";
